...
public class PngWriter extends AbstractWriter<BufferedImage> {
    ...
    @Override
    protected void writeImage(File gpxFile, Path outputFolder, String suffix, BufferedImage image, int width, int height) throws IOException {
        File file;
    ...
    @Override
    protected BufferedImage combineGraphAndMap(GpxStyler styler, BufferedImage map, JFreeChart elevationGraph, int chartHeight) {
        ...
        if (GraphToMapPosition.BOTTOM == styler.graphPosition()) {
            ...
            graphics.drawImage(chartToImage(elevationGraph, styler, map.getHeight(), map.getWidth()), 0, map.getHeight(), null);
        } else {
